...
import {
  ...
  Navigation, NavigationExtras, NavigationStart, ParamMap, Router, LoadedRouterConfig,
} from "@angular/router";
import { flatten, isDate, parseDates } from "./";
...
export function extendRouteConfigWithNavigationExtras(router: Router) {
  router.events.subscribe(event => {
    ...
    if (event instanceof ActivationStart && event.snapshot.data?.extras) {
      ...
      // restore target url on pages- redirect
      if ((event.snapshot.data?.extras as NavigationExtras)?.skipLocationChange) {
        let location = (router as any).location as Location;
        let url = router.getCurrentNavigation()?.extractedUrl.toString();
